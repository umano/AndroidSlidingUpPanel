...
public class ViewDragHelper {
    ...
    public boolean tryStartNestedScroll(View child) {
        return tryCaptureViewForDrag(child, 0);
    }
    ...
    public void consumeNestedPreScroll(int dx, int dy) {
        dragTo(mCapturedView.getLeft() + dx,
                mCapturedView.getTop() + dy,
                dx, dy);
    }

    public void onNestedScroll(int dxUnconsumed,
                               int dyUnconsumed) {
        // If there is unconsumed scroll this means we need to start collapsing
        // IE: We're visible and they're scrolling down from the top of the list
        if (dyUnconsumed > 0) {
            dragTo(mCapturedView.getLeft() + dxUnconsumed,
                    mCapturedView.getTop() + dyUnconsumed,
    ...
    public void onStopNestedScroll() {
        if(mIsInNestedScroll) {
            dispatchViewReleased(mXVelocity, mYVelocity);
            mIsInNestedScroll = false;
        }
    }

    public void consumeNestedPreFling(float velocityX, float velocityY) {
        mXVelocity = velocityX;
        mYVelocity = velocityY;
    }

    public boolean onNestedFling(float velocityX, float velocityY, boolean consumed) {
    ...
    private void saveLastMotion(float x, float y, int pointerId) {
    ...
    public boolean shouldInterceptTouchEvent(MotionEvent ev) {
        if (mIsInNestedScroll) return false;
